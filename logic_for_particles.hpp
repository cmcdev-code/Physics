...
namespace logic_particles {
	...
	inline T get_distance_between_particles(particle<T>& particle_1, particle<T>& particle_2) {
		return sqrt(
			pow(particle_1.get_x_position() - particle_2.get_x_position(), 2) +
			pow(particle_1.get_y_position() - particle_2.get_y_position(), 2) +
			pow(particle_1.get_z_position() - particle_2.get_z_position(), 2)
		);
	}
	template <typename T> 
	...
	template <typename T>
	inline T get_force_from_gravity_x(particle<T>& particle_1, particle<T>& particle_2, double GRAVITY = 0.000000000066743) {
		//using the unit vector but just for the x
		return -(GRAVITY) *particle_1.get_mass()*particle_2.get_mass()* (particle_1.get_i_position() - particle_2.get_i_position()) / pow(get_distance_between_particles(particle_1, particle_2),2);
	}
	#define get_force_from_gravity_i get_force_from_gravity_x

	template <typename T>
	inline T get_force_from_gravity_y(particle<T>& particle_1, particle<T>& particle_2, double GRAVITY = 0.000000000066743) {
		//using the unit vector but just for the y
		return -(GRAVITY)*particle_1.get_mass() * particle_2.get_mass() * (particle_1.get_j_position() - particle_2.get_j_position()) / pow(get_distance_between_particles(particle_1, particle_2), 2);
	}
	#define get_force_from_gravity_j get_force_from_gravity_y

	template <typename T>
	inline T get_force_from_gravity_z(particle<T>& particle_1, particle<T>& particle_2, double GRAVITY = 0.000000000066743) {
		//using the unit vector but just for the z
		return -(GRAVITY)*particle_1.get_mass() * particle_2.get_mass() * (particle_1.get_z_position() - particle_2.get_z_position()) / pow(get_distance_between_particles(particle_1, particle_2), 2);
	}
